...
class CategoryForm(forms.ModelForm):
    title = forms.CharField(max_length=50, widget=forms.TextInput(
        attrs={'type': 'text', 'id': 'category_title', 'class': 'title', 'placeholder': 'Назва категорії', 'required': 'required'}))
...
class DishesForm(forms.ModelForm):
    ...
    description = forms.CharField(max_length=300, widget=forms.TextInput(
        attrs={'type': 'text', 'id': 'title', 'class': 'form-control', 'rows': '4', 'placeholder': 'Опис страви', 'required': 'required'}))
